...
class Groups{
	private ArrayList<Card> single; // cards that are not in any PhaseGroups
	private ArrayList<Card>	cardConflict; // a card that is contained by multiple phaseGroups
	private Card[] cards; // the cards in the hand
	private ArrayList<PhaseGroup> 	complete; // a phasegroup that is validated
	private ArrayList<PhaseGroup>	partial; // a phasegroup that is not validated
	private ArrayList<ArrayList<PhaseGroup>>	conflictingGroups; // PhaseGroups that contain cards that are also in another PhaseGroup
	private AIPlayer player; // The AIPlayer whose hand this is
	...
	/**
	 * compares all partial runs to one another to see if they can be linked
	 */
	private void connectRuns() { 
				...
				if(p1.getType() == Configuration.RUN_PHASE){
					for(PhaseGroup p2: partial){
						if(p2.getType() == Configuration.RUN_PHASE && p1 != p2){
							PhaseGroup temp = new PhaseGroup(player.getGame()); // make a new PhaseGroup, with the two combined
							for(int x = 0; x < p1.getNumberOfCards(); x++)
								temp.addCard(p1.getCard(x));
							for(int x = 0; x < p2.getNumberOfCards(); x++)
								temp.addCard(p2.getCard(x));
							if(PhaseGroup.validate(temp, Configuration.RUN_PHASE, player.lengthOfRunNeeded())){ // if the new group is valid
								partial.remove(p1);// get rid of the old 2
								partial.remove(p2);
								temp.setType(Configuration.RUN_PHASE);
								complete.add(temp); // make it a complete group
								connectRuns(); // keep checking
								return;
							}	
						}
					}
	...
	/**
	 * This method makes sure thier are the right number and types of complete groups
	 */
	private void validateCompleteGroups(){ 
		if(!player.colorPhase()){
			...
			if(completeRuns.size() > 1){ // If we need to get rid of a run
				...
				for(int x = 0; x < completeRuns.size(); x++){
					...
					if(tempValue > highestValue){ // remove the group that had the high value from complete
			...
			if(completeSets.size() > 0 && completeSets.size() != player.setsNeeded().length){//if too few, look to at partial on phases 9 & 10
				...
				if(player.setsNeeded().length > 1 && completeSets.size() < player.setsNeeded().length){ // promote valid partials on phase 9 & 10
					for(PhaseGroup p: partial){
	...
	public PhaseGroup[] getCompletePhaseGroups(){
		if(complete.size() > 0){ // transform from ArrayList to an array (toArray does not work with generics)
	...
	public Card[] recommendDiscard(){ // look at conflicts
		...
		bigLoop: // this loop assigns a "discard value" to each card, the higher the value more likely it will be discarded
			for(int x = 0; x < cards.length; x++){
				...
				// a hard player knows that on these cards will always need to be picked up if this is the case
				if(player.getDifficulty() > 70 && player.lengthOfRunNeeded() > 4){
					if((cards[x].getValue() == 6 || cards[x].getValue() == 7)||
						...
						if(x != 0 && cards[x-1].getValue() != cards[x].getValue()) // check for duplicates
